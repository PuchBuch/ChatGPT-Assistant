...
user_name = 'User'
gpt_name = 'ChatGPT'
# Аватар (формат svg) из https://www.dicebear.com/playground?style=identicon
user_svg = """
...
css_code = """
    ...
    section[data-testid="stSidebar"] h1 {
        ...
        font-family: "Microsoft Yahoo", "auto";
        margin-bottom: 6px;
...
function checkElements() {
    const textinput = window.parent.document.querySelector("textarea[aria-label='**Вход：**']");   //label需要相对应
    ...
    if (textinput && textarea && button && tabs && tabs_div && tab_panels) {
        ...
        window.parent.document.addEventListener('dblclick', function (event) {
            ...
            if (activeTab.querySelector('p').textContent === '💬 Чат') {
                textinput.focus();
            } else {
                tabs[0].click();
                const waitMs = 50;
        ...
        textinput.addEventListener('keydown', function (event) {
            if (event.keyCode === 9) {
                ...
                if (!window.parent.getSelection().toString()) {
                    // Получить текущую позицию плавления
                    const start = this.selectionStart;
                    const end = this.selectionEnd;
                    // Вставить символ табуляции в позицию курсора
                    this.value = this.value.substring(0, start) + '\t' + this.value.substring(end);
                    // Переместите курсор после вставленного символа табуляции
                    this.selectionStart = this.selectionEnd = start + 1;
                }
            }
        });

        // Устранить проблему рендеринга вкладок при первом переключении.
